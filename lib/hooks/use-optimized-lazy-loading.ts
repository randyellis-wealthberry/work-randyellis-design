...
export function useOptimizedLazyLoading({
  ...
  const loadingStrategy = useMemo(() => {
    const isFastConnection = connectionInfo.effectiveType === '4g' && connectionInfo.downlink > 5;
    const isSlowConnection = connectionInfo.effectiveType === '2g' || connectionInfo.effectiveType === 'slow-2g' || connectionInfo.saveData;
    ...
    return {
      ...
      adaptiveMargin: isFastConnection ? "200px" : isSlowConnection ? "50px" : rootMargin,
      adaptiveThreshold: isFastConnection ? 0.001 : isSlowConnection ? 0.1 : threshold
    };
  ...
  const prefetchResource = useCallback((url: string, type: 'image' | 'video' | 'script' | 'style' = 'image') => {
  ...
  const loadResource = useCallback(async (url: string, type: 'image' | 'video' = 'image'): Promise<void> => {
