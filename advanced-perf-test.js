...
class AdvancedPerformanceTest {
  ...
  async testComponentCleanup(browser) {
    ...
    try {
      ...
      await page.goto('http://localhost:3000/metis', { waitUntil: 'networkidle2' });
      await page.waitForSelector('[role="img"][aria-label*="light rays"]', { timeout: 10000 });
      ...
      const memoryLeakMB = initialMemory && finalMemory ? 
        Math.round((finalMemory - initialMemory) / 1024 / 1024) : 0;
      ...
      console.log(`  📊 Memory leak test: ${memoryLeakMB}MB (${this.results.cleanup.status})`);
  ...
  async testReducedMotion(browser) {
    ...
    try {
      ...
      console.log(`  ♿ Reduced motion test: ${this.results.reducedMotion.status}`);
  ...
  async testWebGLContext(browser) {
    ...
    try {
      ...
      console.log(`  🎮 WebGL test: ${this.results.webgl.status} (${webglInfo.hasWebGL ? 'Available' : 'Not available'})`);
  ...
  async testMultipleTabs(browser) {
    ...
    try {
      ...
      await Promise.all(tabs.map(async (page, index) => {
        try {
          await page.goto('http://localhost:3000/metis', { waitUntil: 'networkidle2', timeout: 30000 });
          await page.waitForSelector('[role="img"][aria-label*="light rays"]', { timeout: 10000 });
        } catch (error) {
          console.log(`    Tab ${index + 1} failed to load: ${error.message}`);
        }
      }));
      ...
      const memoryUsages = await Promise.all(tabs.map(async (page) => {
        try {
      ...
      const totalMemory = memoryUsages.filter(m => m !== null).reduce((a, b) => a + b, 0);
      const avgMemoryPerTab = totalMemory / memoryUsages.filter(m => m !== null).length;
      ...
      console.log(`  📑 Multiple tabs test: ${this.results.multipleTabs.status} (${this.results.multipleTabs.successfulTabs}/${tabs.length} loaded)`);
  ...
  async testSlowNetwork(browser) {
    ...
    try {
      ...
      await page.goto('http://localhost:3000/metis', { waitUntil: 'networkidle2', timeout: 60000 });
      ...
      console.log(`  🐌 Slow network test: ${this.results.slowNetwork.status} (${loadTime}ms)`);
  ...
  generateAdvancedReport() {
    ...
    const passedTests = Object.values(this.results)
      .filter(r => r.status === 'good' || r.status === 'excellent').length;
    ...
    switch (testKey) {
      case 'cleanup':
        if (result.memoryLeakMB !== undefined) {
          ...
          console.log(`   Before/After: ${result.initialMemoryMB}MB → ${result.finalMemoryMB}MB`);
        }
        break;
        
      case 'reducedMotion':
        console.log(`   Respects Preference: ${result.respectsPreference ? '✅' : '❌'}`);
      ...
      case 'multipleTabs':
        if (result.tabCount) {
          console.log(`   Tabs Loaded: ${result.successfulTabs}/${result.tabCount}`);
      ...
      case 'slowNetwork':
        if (result.loadTime) {
          ...
          console.log(`   LightRays Loaded: ${result.lightRaysLoaded ? '✅' : '❌'}`);
        }
        break;
    }
  ...
  generateAdvancedRecommendations() {
      ...
      switch (testKey) {
        case 'cleanup':
          if (result.status === 'warning' && result.memoryLeakMB > 5) {
            recommendations.push(`🧹 Memory leak detected (${result.memoryLeakMB}MB). Implement proper WebGL context cleanup.`);
          }
          break;
          
        case 'reducedMotion':
          if (result.status === 'needs-improvement') {
            recommendations.push(`♿ Add prefers-reduced-motion support to disable animations for accessibility.`);
          }
          break;
          
        case 'webgl':
          if (result.status === 'warning') {
            recommendations.push(`🎮 WebGL not available. Ensure graceful fallback to static visual effects.`);
          }
          break;
          
        case 'multipleTabs':
          if (result.status === 'warning') {
            recommendations.push(`📑 Performance degrades with multiple tabs. Consider context sharing or reduced quality.`);
          }
          break;
          
        case 'slowNetwork':
          if (result.status === 'warning') {
            recommendations.push(`🐌 Poor performance on slow networks. Consider lazy loading or progressive enhancement.`);
          }
          break;
      }
    });
    ...
    const goodTests = Object.entries(this.results)
      .filter(([_, result]) => result.status === 'good' || result.status === 'excellent')
      .map(([key, _]) => key);
    
    if (goodTests.includes('cleanup')) {
      recommendations.push('✅ Clean component unmounting prevents memory leaks.');
    }
    
    if (goodTests.includes('reducedMotion')) {
      recommendations.push('✅ Accessibility compliance with reduced motion preferences.');
    }
    
    if (goodTests.includes('webgl')) {
      recommendations.push('✅ WebGL context properly initialized and functional.');
    }
    
    if (recommendations.length === 0) {
      recommendations.push('🎉 All advanced tests passed! LightRays implementation is robust.');
    }
