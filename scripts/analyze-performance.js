...
async function identifyOptimizationOpportunities(analysis) {
  ...
  const largeChunks = analysis.bundles.filter(
    (b) => b.size > BUNDLE_LIMITS.maxChunkSize,
  );
  ...
  if (analysis.totalSize > BUNDLE_LIMITS.maxTotalJSSize) {
    opportunities.push({
      ...
      implementation:
        "Implement lazy loading for 3D components and tree shaking",
    });
  }
  ...
  if (potentialThreeJSChunk) {
    opportunities.push({
      ...
      priority: "High",
      description:
        "Three.js and related libraries should be dynamically imported",
      ...
      implementation:
        "Move Three.js to separate chunk with intersection observer loading",
    });
  }
...
function printReport(report) {
  ...
  console.log(
    `Exceeds targets: ${report.summary.exceedsTargets ? "❌ Yes" : "✅ No"}\n`,
  );
  ...
  console.log(
    `Bundle size: ${report.performanceTargets.current.bundleSize}KB → ${report.performanceTargets.targets.bundleSize}KB`,
  );
  console.log(
    `Largest chunk: ${report.performanceTargets.current.largestChunk}KB → ${report.performanceTargets.targets.largestChunk}KB`,
  );
  console.log(
    `Est. LCP: ${report.performanceTargets.current.estimatedLCP}ms → ${report.performanceTargets.targets.estimatedLCP}ms`,
  );
  console.log(
    `Est. TTI: ${report.performanceTargets.current.estimatedTTI}ms → ${report.performanceTargets.targets.estimatedTTI}ms\n`,
  );
