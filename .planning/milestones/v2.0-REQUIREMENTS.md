# Requirements Archive — v2.0 Case-Study Depth

**Status:** ✅ SHIPPED 2026-08-22
**Audit:** [v2.0-MILESTONE-AUDIT.md](v2.0-MILESTONE-AUDIT.md) — `passed_with_notes`
**Outcome:** 30/31 satisfied by direct verification · 1 gap carried to v3.0 ·
4 satisfied by a superseding architecture (restated in place below)
...
> **Restated at close.** These were written against four separate components
> (`DecisionCallout`, `CaseStudyTOC`, `ReflectionBlock`, `RoleNarrativeSection`).
> Nothing imports those names. What shipped is a single shared
> `components/case-study/case-study-template.tsx` plus `diagrams` and
> `section-chrome`, imported by all five standalone project pages — addvanced,
> echo, nagarro, rambis-ui **and waffle**. `CaseStudyTemplate` renders the
> decision-with-rationale arc (14 `decision`/`rationale` references).
>
> The phase goal was one structurally consistent pattern across the case
> studies. One template achieves that more strongly than four composed
> components, and pulled waffle in as a bonus. Recorded here as shipped, so the
> archive matches the code rather than the original spec.

- [x] **MIG-01**: addvanced converged onto the shared `CaseStudyTemplate` — *shipped as a single template, not the four specified components*
- [x] **MIG-02**: echo converged onto the shared template via the live `echo-client` (the dead `-final` variant was never touched)
- [x] **MIG-03**: nagarro converged onto the shared template
- [x] **MIG-04**: rambis-ui converged onto the shared template
...
- [~] **CRED-07** *(gap — carried to v3.0)*: Every decision/metric/outcome comes from Randy's firsthand account; the writer never invents one to fill a narrative slot. Figures a reader can independently check (store ratings, download counts) should be right, and the site must not contradict itself across surfaces.
  **Holds within case-study content; fails sitewide.** FND-03's own deck audit
  ruled SITE-01 (`2.5M+ users`), SITE-03 (`$50M product value`) and SITE-04
  (`800+ designers mentored`) `Unbacked`, and all three remain live in
  `components/core/animated-number-basic.tsx:12-15`, propagating to `/about`,
  `/services`, `lib/metadata.ts` and both OG generators. Phase 9's `09-03-PLAN`
  aligned the `$50M` *wording* rather than removing the claim. Scoped into v3.0
  as CRED-10..12. Note "4 Design Awards" shares the same component and **is**
  backed (deck slide 28) — it must survive remediation.
...
| MIG-01 | Phase 7 | Complete (restated) |
| MIG-02 | Phase 7 | Complete (restated) |
| MIG-03 | Phase 7 | Complete (restated) |
| MIG-04 | Phase 7 | Complete (restated) |
...
| CRED-07 | Phase 8 | Gap → v3.0 CRED-10..12 |
...
**Verification note:** Phases 06, 07 and 08 shipped without GSD artifacts —
phases 07 and 08 have no directory and no git history. Their requirements were
verified directly against the codebase at milestone close, the same compensation
v1.0 applied to its phases 1-3. See the audit.
...
*Archived: 2026-08-22 at v2.0 close*
