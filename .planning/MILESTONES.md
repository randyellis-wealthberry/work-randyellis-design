...
**Known deferred items at close:** ~~CRED-07 sitewide gap — SITE-01 (`2.5M+`), SITE-03 (`$50M`), SITE-04 (`800+`) were ruled `Unbacked` … Scoped into v3.0 as CRED-10..12.~~ **CORRECTED 2026-08-22 (REC-01): this was never a gap.** `DECK-COVERAGE-AUDIT.md:34` defines `Unbacked` as *"not found in this deck. **This is not a defect and requires no action**"*, and `:37` records that the non-deck-source rule was removed because *"Randy's own account of his engagements is a source."* This entry restated those descriptive verdicts as credibility debt, and v3.0 was staged around removing three accurate figures on that basis. The figures stay; the removal requirements were voided. What was genuinely open: `app/about/opengraph-image.tsx:238` still renders `6` Design Awards against 4 everywhere else (now CRED-13). Also: blog Article dates lack time/timezone and an `image`; `Paywalled Content` detected on a non-paywalled post; `/services` reader conflict unresolved; phases 07-08 lack GSD artifacts (compensated by direct-codebase audit, as v1.0 did for phases 1-3).
