...
const ComponentLoader = ({ 
  ...
  const getMessage = () => {
    switch (type) {
      case "webgl": return "Loading 3D content...";
      case "video": return "Loading video player...";
      case "carousel": return "Loading carousel...";
  ...
  return (
    <div className={cn(
      "w-full h-full flex items-center justify-center bg-muted/10",
...
export const LazyAnimatedWebGL = (props: any) => (
  <Suspense fallback={<ComponentLoader type="webgl" className={props.className} />}>
    <DynamicAnimatedWebGL {...props} />
  </Suspense>
);

export const LazyTestimonialCarousel = (props: any) => (
  <Suspense fallback={<ComponentLoader type="carousel" className={props.className} />}>
    <DynamicTestimonialCarousel {...props} />
  </Suspense>
);

export const LazyVideoPlayer = (props: any) => (
  <Suspense fallback={<ComponentLoader type="video" className={props.className} />}>
...
export function createIntersectionLazyComponent<T = any>(
  ...
  return (props: T & { className?: string; containerClassName?: string }) => {
    return (
      <Suspense fallback={<ComponentLoader type={fallbackType} className={props.className} />}>
        <LazyComponent {...props} />
      </Suspense>
    );
  };
