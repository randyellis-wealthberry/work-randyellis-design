...
export const OptimizedLazyVideo = forwardRef<HTMLDivElement, OptimizedLazyVideoProps>(({
  src,
  poster,
  className,
  containerClassName,
  ...
  const adaptivePreload = enableAdaptiveQuality 
    ? connectionInfo.effectiveType === '4g' ? 'metadata'
    : connectionInfo.effectiveType === '3g' ? 'none'
    : 'none'
    : preload;

  // Prefetch video when appropriate
  useEffect(() => {
    if (shouldPreload && loadingStrategy.aggressivePreload && connectionInfo.effectiveType === '4g') {
      prefetchResource(src, 'video');
    }
  }, [shouldPreload, loadingStrategy.aggressivePreload, prefetchResource, src, connectionInfo]);
  ...
  useEffect(() => {
    if (videoRef.current && isInView && videoLoaded && autoPlay && !isPlaying) {
  ...
  const renderSkeleton = () => (
    <div
      className={cn(
        "animate-pulse bg-muted/20 rounded flex items-center justify-center aspect-video",
        className
      )}
    >
  ...
  const renderError = () => (
    <div
      className={cn(
        "flex items-center justify-center bg-destructive/5 text-destructive rounded border-2 border-dashed border-destructive/20 aspect-video",
        className
      )}
    >
      <div className="text-center p-4">
        <div className="text-sm font-medium">Failed to load video</div>
        <div className="text-xs opacity-70 mt-1">Check your connection and try again</div>
      </div>
    </div>
  );
