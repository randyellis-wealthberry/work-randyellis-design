...
class PerformanceProfiler {
  ...
  async testDevice(deviceName, viewport) {
    ...
    try {
      ...
      const animationMetrics = await this.measureAnimationPerformance(page, deviceName);
      ...
      this.results.devices[deviceName] = {
        ...
        score: this.calculateDeviceScore(performanceMetrics, memoryMetrics, webglMetrics, animationMetrics)
      };
  ...
  async measureMemoryUsage(page, deviceName) {
    ...
    await page.waitForFunction(() => new Promise(resolve => setTimeout(resolve, 5000))); // Let animations run
    ...
    const memoryGrowth = finalMemory && initialMemory ? 
      (finalMemory.usedJSHeapSize - initialMemory.usedJSHeapSize) / (1024 * 1024) : 0;
      ...
      return new Promise((resolve) => {
        ...
        function measureFrame() {
          ...
          if (frameCount < maxFrames) {
            ...
            const droppedFrames = frameTimes.filter(time => time > 16.67).length; // Frames > 60fps threshold
            
            resolve({
              averageFPS: Math.round(fps),
              droppedFrames,
              frameTimeConsistency: Math.round((1 - (droppedFrames / frameCount)) * 100),
  ...
  calculateDeviceScore(performance, memory, webgl, animation) {
    ...
    if (performance.totalLoadTime > config.targetMetrics.pageLoadTime) {
      const penalty = Math.min(20, (performance.totalLoadTime - config.targetMetrics.pageLoadTime) / 100);
      score -= penalty;
    ...
    if (memory.growthMB > config.targetMetrics.memoryGrowth) {
      const penalty = Math.min(15, (memory.growthMB - config.targetMetrics.memoryGrowth) / 5);
      score -= penalty;
    ...
    if (animation.averageFPS < config.targetMetrics.fps) {
      const penalty = Math.min(25, (config.targetMetrics.fps - animation.averageFPS) / 2);
      score -= penalty;
  ...
  async generateReport() {
    ...
    const averageScore = deviceScores.length > 0 ? 
    ...
    Object.entries(this.results.devices).forEach(([deviceName, deviceData]) => {
      if (deviceData.score && deviceData.score.total < 60) {
        this.results.summary.critical.push(`${deviceName}: Poor performance (${deviceData.score.total}/100)`);
      }
      
      if (deviceData.memory && deviceData.memory.growthMB > config.targetMetrics.memoryGrowth * 1.5) {
        this.results.summary.critical.push(`${deviceName}: Excessive memory usage (${deviceData.memory.growthMB.toFixed(1)}MB)`);
      }
      
      if (deviceData.animation && deviceData.animation.averageFPS < 30) {
        this.results.summary.critical.push(`${deviceName}: Very low FPS (${deviceData.animation.averageFPS})`);
      }
    });
  ...
  generateRecommendations() {
    ...
    Object.entries(this.results.devices).forEach(([deviceName, deviceData]) => {
      if (deviceData.performance && deviceData.performance.totalLoadTime > config.targetMetrics.pageLoadTime) {
        recommendations.push({
          priority: 'high',
          device: deviceName,
          issue: 'Slow page load',
          recommendation: 'Consider implementing code splitting or reducing initial bundle size'
        });
      }
      
      if (deviceData.memory && deviceData.memory.growthMB > config.targetMetrics.memoryGrowth) {
        recommendations.push({
          priority: 'medium',
          device: deviceName,
          issue: 'Memory growth',
          recommendation: 'Implement proper cleanup in WebGL components and check for memory leaks'
        });
      }
      ...
      if (deviceData.animation && deviceData.animation.averageFPS < 50) {
        recommendations.push({
          ...
          device: deviceName,
          issue: 'Low FPS',
          recommendation: 'Reduce animation complexity or implement performance scaling based on device capabilities'
        });
      }
    });
  ...
  printSummary() {
    ...
    console.log(`🎯 Overall Score: ${this.results.summary.overallScore}/100 (Grade: ${this.results.summary.overallGrade})`);
    console.log(`✅ Devices Passed: ${this.results.summary.passedDevices}/${this.results.summary.totalDevices}`);
    
    if (this.results.summary.critical.length > 0) {
      console.log('\n🚨 CRITICAL ISSUES:');
      this.results.summary.critical.forEach(issue => console.log(`  • ${issue}`));
    }
    
    console.log('\n📱 DEVICE BREAKDOWN:');
    Object.entries(this.results.devices).forEach(([device, data]) => {
      if (data.score) {
        console.log(`  ${device.toUpperCase()}: ${data.score.total}/100 (${data.score.grade})`);
        if (data.performance) {
        ...
        if (data.memory) {
          console.log(`    🧠 Memory Growth: ${data.memory.growthMB.toFixed(1)}MB`);
        }
      }
    });
    ...
    if (this.results.summary.recommendations.length > 0) {
      ...
      this.results.summary.recommendations.forEach((rec, index) => {
        console.log(`  ${index + 1}. [${rec.priority.toUpperCase()}] ${rec.device}: ${rec.issue}`);
