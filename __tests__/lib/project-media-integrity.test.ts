...
describe('Project Media Integrity', () => {
  describe('Unique Media Paths', () => {
    it('should ensure all projects have unique video paths', () => {
      const videoPaths = PROJECTS
        .filter(project => project.video && project.video.trim() !== '')
        .map(project => project.video);
      
      const uniqueVideoPaths = new Set(videoPaths);
      
      // Find duplicates for better error reporting
      const duplicates = videoPaths.filter((path, index) => 
        videoPaths.indexOf(path) !== index
      );
      ...
      if (duplicates.length > 0) {
        ...
        throw new Error(`Found duplicate video paths:\n${duplicateInfo.join('\n')}`);
      }
    ...
    it('should ensure all projects have unique thumbnail paths when defined', () => {
      const thumbnailPaths = PROJECTS
        .filter(project => project.thumbnail && project.thumbnail.trim() !== '')
        .map(project => project.thumbnail);
      
      const uniqueThumbnailPaths = new Set(thumbnailPaths);
      
      // Find duplicates for better error reporting
      const duplicates = thumbnailPaths.filter((path, index) => 
        thumbnailPaths.indexOf(path) !== index
      );
      ...
      if (duplicates.length > 0) {
        ...
        throw new Error(`Found duplicate thumbnail paths:\n${duplicateInfo.join('\n')}`);
      }
    ...
    it('should not have projects sharing the same video and thumbnail combination', () => {
      ...
      if (duplicatedCombos.length > 0) {
        ...
        throw new Error(`Found projects sharing the same media combination:\n${duplicateInfo.join('\n')}`);
      }
  ...
  describe('Project Media Validation', () => {
    ...
    it('should ensure video and thumbnail paths are properly formatted', () => {
      ...
      PROJECTS.forEach(project => {
        if (project.video) {
          if (!project.video.startsWith('/') && !project.video.startsWith('http')) {
            invalidPaths.push(`${project.name}: Invalid video path format - ${project.video}`);
          }
        }
        
        if (project.thumbnail) {
          if (!project.thumbnail.startsWith('/') && !project.thumbnail.startsWith('http')) {
            invalidPaths.push(`${project.name}: Invalid thumbnail path format - ${project.thumbnail}`);
          }
        }
      });
      
      if (invalidPaths.length > 0) {
        throw new Error(`Found invalid media path formats:\n${invalidPaths.join('\n')}`);
      }
