...
describe("TestResults Component", () => {
  describe("Component Rendering", () => {
    it("should render the component title", () => {
      ...
      expect(
        screen.getByRole("heading", { name: /test results/i }),
      ).toBeInTheDocument();
    });
  ...
  describe("Timestamp Formatting", () => {
    ...
    it("should handle invalid timestamps gracefully", () => {
      ...
      expect(() =>
        render(<TestResults results={[invalidResult]} />),
      ).not.toThrow();
    });
  });
  ...
  describe("Expandable Details", () => {
    it('should show "View Details" button only when details exist', () => {
      ...
      const firstResultContainer = screen
        .getByText("Submitting email to newsletter endpoint...")
        .closest("div");
      expect(
        within(firstResultContainer!).queryByText("View Details"),
      ).not.toBeInTheDocument();

      // Second result has details
      const secondResultContainer = screen
        .getByText(
          "API Error: Server configuration error. Please contact support.",
        )
        .closest("div");
      expect(
        within(secondResultContainer!).getByText("View Details"),
      ).toBeInTheDocument();
    });
    ...
    it("should handle string details correctly", async () => {
      ...
      expect(
        screen.getByText(
          "Connection established to PostgreSQL database at port 5432",
        ),
      ).toBeInTheDocument();
    });
    ...
    it("should format JSON details correctly", async () => {
      ...
      expect(screen.getByText('"error"')).toBeInTheDocument();
      expect(
        screen.getByText('"JWT token validation failed"'),
      ).toBeInTheDocument();
  ...
  describe("Edge Cases", () => {
    it("should handle missing required properties gracefully", () => {
      ...
      expect(() =>
        render(<TestResults results={[incompleteResult]} />),
      ).not.toThrow();
    });
    ...
    it("should handle large datasets without performance issues", () => {
      const largeDataset: TestResult[] = Array.from(
        { length: 100 },
        (_, i) => ({
          id: `result-${i}`,
          title: `Test Result ${i}`,
