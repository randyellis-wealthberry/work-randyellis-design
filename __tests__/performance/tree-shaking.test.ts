...
describe("Tree Shaking Performance Tests", () => {
  ...
  test("Three.js should use selective imports instead of full library", () => {
    ...
    const filesWithFullThreeImport = threeJSFiles.filter((file) => {
      ...
      return (
        content.includes('import * as THREE from "three"') ||
    ...
    if (filesWithFullThreeImport.length > 0) {
      console.log(
        "Files with full Three.js imports:",
        filesWithFullThreeImport.map((f) => f.filePath),
      );
    }
  });
  ...
  test("React Three Fiber components should be selectively imported", () => {
    ...
    const filesWithFullR3FImport = r3fFiles.filter((file) => {
      ...
      return (
        content.includes("import * as") && content.includes("@react-three")
      );
    });
  ...
  test("Framer Motion should use selective imports", () => {
    const motionFiles = importAnalysis.filter((file) =>
      file.heavyImports.some(
        (imp) => imp.includes("motion") || imp.includes("framer-motion"),
      ),
    );
    ...
    if (problematicImports.length > 0) {
      console.log(
        "Files with problematic motion imports:",
        problematicImports.map((f) => f.filePath),
      );
    }
  });
  ...
  test("Radix UI components should be imported from specific packages", () => {
    ...
    const filesWithSuboptimalRadixImports = radixFiles.filter((file) => {
      ...
      return (
        content.includes('from "@radix-ui/react"') ||
  ...
  test("Bundle should not include unused library code", () => {
    ...
    const filesWithUnusedImports = importAnalysis.filter(
      (file) => file.unusedImports.length > 0,
    );
    ...
    if (heavyUnusedImports.length > 0) {
      console.log(
        "Files with unused heavy imports:",
        heavyUnusedImports.map(
          (f) => `${f.filePath}: ${f.unusedImports.join(", ")}`,
        ),
      );
    }
  });
  ...
  test("Dynamic imports should be used for heavy 3D components", () => {
    const componentFiles = importAnalysis.filter(
      (file) =>
...
function findTreeShakingOpportunities(
  content: string,
  imports: string[],
): string[] {
...
function generateTreeShakingReport(
  ...
  const filesWithUnusedImports = analysis.filter(
    (a) => a.unusedImports.length > 0,
  );
...
function calculateTreeShakingEfficiency(analysis: ImportAnalysis[]) {
  const totalImports = analysis.reduce(
    (sum, file) => sum + file.totalImports,
    0,
  );
  const totalUnusedImports = analysis.reduce(
    (sum, file) => sum + file.unusedImports.length,
    0,
  );
  const heavyLibraryFiles = analysis.filter(
    (file) => file.heavyImports.length > 0,
  );
  const optimizedHeavyFiles = heavyLibraryFiles.filter(
    (file) => file.potentialTreeShaking.length === 0,
  );
  const selectiveImportFiles = analysis.filter(
    (file) =>
      !file.potentialTreeShaking.some((opp) =>
        opp.includes("Consider selective"),
      ),
  );

  return {
    unusedImportPercentage:
      totalImports > 0 ? (totalUnusedImports / totalImports) * 100 : 0,
    heavyLibraryOptimization:
      heavyLibraryFiles.length > 0
        ? (optimizedHeavyFiles.length / heavyLibraryFiles.length) * 100
        : 100,
    selectiveImportUsage:
      analysis.length > 0
        ? (selectiveImportFiles.length / analysis.length) * 100
        : 100,
  };
}
