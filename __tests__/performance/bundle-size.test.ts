...
describe('Bundle Size Performance Tests', () => {
  ...
  test('Main bundle size should be under limit', () => {
    const mainBundle = bundleStats.find(bundle => 
      bundle.fileName.includes('4bd1b696') || // Current main chunk
      bundle.fileName.includes('main-')
    );
    
    if (mainBundle) {
      expect(mainBundle.size).toBeLessThanOrEqual(BUNDLE_LIMITS.maxMainBundleSize);
    } else {
  ...
  test('No individual chunk should exceed size limit', () => {
    ...
    if (oversizedChunks.length > 0) {
      console.log('Oversized chunks:', oversizedChunks.map(c => `${c.fileName}: ${c.size}KB`));
  ...
  test('Framework chunks should be within limits', () => {
    const frameworkChunks = bundleStats.filter(bundle => 
    ...
    frameworkChunks.forEach(chunk => {
      expect(chunk.size).toBeLessThanOrEqual(BUNDLE_LIMITS.maxFrameworkChunkSize);
    });
  });
...
describe('Bundle Size Regression Tests', () => {
  test('Bundle sizes should not increase significantly', () => {
    ...
    expect(bundleSizeBaseline.maxObservedMainBundle).toBeGreaterThan(BUNDLE_LIMITS.maxMainBundleSize);
    expect(bundleSizeBaseline.maxObservedLargestChunk).toBeGreaterThan(BUNDLE_LIMITS.maxChunkSize);
  });
});
