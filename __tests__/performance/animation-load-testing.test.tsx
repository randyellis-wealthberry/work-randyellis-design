...
class AnimationPerformanceMonitor {
  ...
  getAverageAnimationDuration() {
    ...
    return (
      this.animationDurations.reduce((sum, duration) => sum + duration, 0) /
      this.animationDurations.length
    );
  }
...
const HeavyAnimationComponent = ({
  ...
  const elements = React.useMemo(
    () => LoadTestUtils.createMultipleElements(elementCount),
    [elementCount],
  );
  ...
  return <div data-testid="heavy-animation-container">{animatedElements}</div>;
};
...
describe("Animation Performance Testing Under Load", () => {
  ...
  describe("Concurrent Animation Performance", () => {
    test("should handle multiple animation types simultaneously", async () => {
      ...
      await act(async () => {
        // Mouse movements
        const events = LoadTestUtils.createStressfulMouseEvents(
          document.body,
          30,
        );
        events.forEach((event, index) => {
  ...
  describe("Performance Recovery and Adaptation", () => {
    test("should recover performance after load spike", async () => {
      ...
      const AdaptiveAnimationComponent = () => {
        const [elementCount, setElementCount] =
          React.useState(currentElementCount);
