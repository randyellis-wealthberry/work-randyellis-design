...
describe("CSP Violations", () => {
  ...
  describe("Violation Report Processing", () => {
    it("should extract relevant information from violation reports", () => {
      ...
      expect(report["blocked-uri"]).toBe(
        "https://suspicious-domain.com/malware.js",
      );
      expect(report["document-uri"]).toBe(
        "https://work.randyellis.design/projects",
      );
    ...
    it("should categorize violations by type", () => {
      const violations = [
        ...
        },
        {
          ...
          "blocked-uri": "https://tracking.com/beacon",
        },
      ];

      const categories = {
        script: violations.filter((v) =>
          v["violated-directive"].includes("script"),
        ),
        style: violations.filter((v) =>
          v["violated-directive"].includes("style"),
        ),
        image: violations.filter((v) =>
          v["violated-directive"].includes("img"),
        ),
        connect: violations.filter((v) =>
          v["violated-directive"].includes("connect"),
        ),
      };
    ...
    it("should identify potentially dangerous violations", () => {
      ...
      dangerousViolations.forEach((violation) => {
        const isDangerous =
          violation["violated-directive"].includes("script") ||
  ...
  describe("Violation Monitoring", () => {
    it("should track violation frequency", () => {
      const violations = [
        ...
        {
          directive: "img-src",
          uri: "https://other.com/image.jpg",
          timestamp: Date.now(),
        },
      ];

      const frequencies = violations.reduce(
        (acc, violation) => {
    ...
    it("should identify repeat offender domains", () => {
      ...
      const domainCounts = violations.reduce(
        (acc, violation) => {
          try {
