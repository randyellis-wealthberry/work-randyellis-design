  ...
  isAllowedSource: (src: string) => {
    ...
    try {
      ...
      return patterns.some(pattern => {
        return pattern.protocol === url.protocol.slice(0, -1) &&
...
const generateOptimizedImageAttributes = ({
  ...
  const optimizedSrc = imageOptimization.generateOptimizedUrl(src, Number(width), quality);
...
describe('CDN Image Optimization', () => {
  ...
  describe('Remote Patterns', () => {
    ...
    it('should allow Cosmos CDN images', () => {
      ...
      const cosmosPattern = patterns.find(p => p.hostname === 'cdn.cosmos.so');
    ...
    it('should allow own domain images', () => {
      ...
      const ownDomainPattern = patterns.find(p => p.hostname === 'work.randyellis.design');
    ...
    it('should validate allowed sources correctly', () => {
      expect(imageOptimization.isAllowedSource('https://images.unsplash.com/photo-123')).toBe(true);
      expect(imageOptimization.isAllowedSource('https://cdn.cosmos.so/image.jpg')).toBe(true);
      expect(imageOptimization.isAllowedSource('https://work.randyellis.design/logo.png')).toBe(true);
      expect(imageOptimization.isAllowedSource('/local-image.jpg')).toBe(true);
      expect(imageOptimization.isAllowedSource('https://evil.com/malicious.jpg')).toBe(false);
    });
  });
  ...
  describe('URL Generation', () => {
    ...
    it('should generate URLs with only quality when width not specified', () => {
      ...
      const optimizedUrl = imageOptimization.generateOptimizedUrl(src, undefined, 80);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle relative paths', () => {
      ...
      expect(imageOptimization.isAllowedSource('../images/relative.jpg')).toBe(false);
      expect(imageOptimization.isAllowedSource('./images/dot-relative.jpg')).toBe(false);
    });
    
    it('should handle edge case dimensions', () => {
      const verySmall = imageOptimization.generateOptimizedUrl('/test.jpg', 1, 1);
      const veryLarge = imageOptimization.generateOptimizedUrl('/test.jpg', 10000, 100);
