...
describe("CDN Cache Headers", () => {
  describe("Static Assets", () => {
    it("should set long-term cache headers for JavaScript files", () => {
      const request = {
        nextUrl: { pathname: "/_next/static/chunks/app.js" },
      } as NextRequest;
      ...
      expect(result.headers.get("CDN-Cache-Control")).toBe(
  ...
  describe("Image Assets", () => {
    it("should set optimized cache headers for images with stale-while-revalidate", () => {
      const request = {
        nextUrl: { pathname: "/images/profile.jpg" },
      } as NextRequest;
      const response = new NextResponse();
    ...
    it("should handle WebP images with appropriate headers", () => {
      const request = {
        nextUrl: { pathname: "/images/hero.webp" },
      } as NextRequest;
      const response = new NextResponse();

      const result = applyCacheHeaders(request, response);

      expect(result.headers.get("Cache-Control")).toContain(
    ...
    it("should handle AVIF images with appropriate headers", () => {
      const request = {
        nextUrl: { pathname: "/images/banner.avif" },
      } as NextRequest;
      const response = new NextResponse();
  ...
  describe("Font Assets", () => {
    it("should set long-term cache headers for WOFF2 fonts", () => {
      const request = {
        nextUrl: { pathname: "/fonts/geist.woff2" },
      } as NextRequest;
      const response = new NextResponse();
    ...
    it("should include CORS headers for fonts", () => {
      const request = {
        nextUrl: { pathname: "/fonts/geist-mono.woff" },
      } as NextRequest;
      const response = new NextResponse();
  ...
  describe("API Routes", () => {
    ...
    it("should handle newsletter API with appropriate caching", () => {
      const request = {
        nextUrl: { pathname: "/api/newsletter" },
      } as NextRequest;
      const response = new NextResponse();
  ...
  describe("Cache Invalidation Strategies", () => {
    ...
    it("should implement stale-while-revalidate for appropriate content", () => {
      ...
      expect(imageHeaders["Cache-Control"]).toContain(
        "stale-while-revalidate=86400",
      );
      expect(dynamicHeaders["Cache-Control"]).toContain(
