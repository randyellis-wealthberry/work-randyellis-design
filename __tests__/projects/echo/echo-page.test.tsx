...
describe('EchoDriveCaseStudy Page', () => {
  describe('Component Rendering', () => {
    ...
    it('includes the client component', () => {
      ...
      expect(screen.getByText('AI-Powered Cloud Intelligence Platform')).toBeInTheDocument();
    });
  });

  describe('Metadata Validation', () => {
    it('has correct title metadata', () => {
      expect(metadata.title).toBe('EchoDrive Case Study | AI-Powered Smart Cloud Storage');
    });
    ...
    it('has proper OpenGraph metadata', () => {
      expect(metadata.openGraph).toEqual({
        ...
        description: 'AI-powered cloud storage case study: 35% storage efficiency, 5x search speed, 89% user adoption through intelligent file management.',
        type: 'article',
        images: [
          {
    ...
    it('has proper Twitter metadata', () => {
      expect(metadata.twitter).toEqual({
        ...
        description: 'How AI-powered intelligence transformed cloud storage efficiency and collaboration workflows.',
    ...
    it('emphasizes AI-powered aspects in all metadata', () => {
      ...
      expect(metadata.twitter?.description).toContain('AI-powered intelligence');
    });

    it('uses consistent image paths across metadata', () => {
      const expectedImagePath = '/projects/echo/img1.jpg';
      
      expect(metadata.openGraph?.images?.[0]?.url).toBe(expectedImagePath);
      expect(metadata.twitter?.images?.[0]).toBe(expectedImagePath);
    });
    ...
    it('has descriptive alt text for images', () => {
      const ogImage = metadata.openGraph?.images?.[0];
      expect(ogImage?.alt).toBe('EchoDrive AI-powered cloud storage interface');
    });
  });
  ...
  describe('Image Optimization', () => {
    it('uses optimized image paths in metadata', () => {
      const imagePath = metadata.openGraph?.images?.[0]?.url;
    ...
    it('has proper image dimensions for social sharing', () => {
      ...
      const aspectRatio = (ogImage?.width || 0) / (ogImage?.height || 1);
