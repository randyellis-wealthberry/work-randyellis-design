  ...
  const createMotionComponent = (element: string) => {
    return React.forwardRef(({ children, ...props }: any, ref: any) => {
      const { 
        ...
        onAnimationComplete, onAnimationStart, whileHover, 
        whileTap, whileInView, drag, dragConstraints, 
        layoutId, layout, style, ...domProps 
      } = props;
...
const MetricCard: React.FC<MetricCardProps> = ({ metric, index, onCelebrate }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-full w-full">
        ...
        {hasAnimated && (
          <div data-testid="celebration-cloud">☁️</div>
        )}
      </div>
    </div>
  );
...
describe('MetricCard Component', () => {
  ...
  describe('Rendering and Basic Functionality', () => {
    it('renders metric card with correct content', () => {
      render(<MetricCard metric={mockMetric} index={0} onCelebrate={mockOnCelebrate} />);
    ...
    it('does not display achievement badge for low percentage values', () => {
      ...
      render(<MetricCard metric={lowMetric} index={0} onCelebrate={mockOnCelebrate} />);
    ...
    it('handles non-percentage values correctly', () => {
      ...
      render(<MetricCard metric={speedMetric} index={0} onCelebrate={mockOnCelebrate} />);
  ...
  describe('Accessibility Features', () => {
    it('has proper ARIA labels and roles', () => {
      ...
      expect(card).toHaveAttribute('aria-label', 'Storage Efficiency metric: 35%. Click to highlight this achievement.');
    });
    ...
    it('responds to keyboard navigation', async () => {
      ...
      expect(screen.getByTestId('metric-value')).toHaveAttribute('data-heartbeat', 'true');
      
      // Wait for heartbeat to finish
      act(() => {
        jest.advanceTimersByTime(600);
      });
      expect(screen.getByTestId('metric-value')).toHaveAttribute('data-heartbeat', 'false');
    });
    ...
    it('has proper ARIA label on animated number', () => {
      ...
      expect(animatedNumber).toHaveAttribute('aria-label', 'Storage Efficiency: 35%');
    });
  });
  ...
  describe('Interactive Behavior', () => {
    ...
    it('triggers celebration callback after animation delay', () => {
      render(<MetricCard metric={mockMetric} index={2} onCelebrate={mockOnCelebrate} />);
  ...
  describe('Animation States', () => {
    it('starts with no animation state', () => {
      ...
      expect(screen.getByTestId('metric-card-0')).toHaveAttribute('data-has-animated', 'false');
    });
    ...
    it('updates animation state after timer completes', () => {
      ...
      expect(screen.getByTestId('metric-card-0')).toHaveAttribute('data-has-animated', 'true');
    });
  ...
  describe('Staggered Animation Timing', () => {
    ...
    it('applies different delays for higher indices', () => {
      ...
      render(<MetricCard metric={mockMetric} index={3} onCelebrate={mockCelebrate} />);
  ...
  describe('Performance and Memory Management', () => {
    it('cleans up timers on unmount', () => {
      const { unmount } = render(
        <MetricCard metric={mockMetric} index={0} onCelebrate={mockOnCelebrate} />
      );
  ...
  describe('Edge Cases', () => {
    it('handles malformed metric values gracefully', () => {
      ...
      expect(() => {
        render(<MetricCard metric={malformedMetric} index={0} onCelebrate={mockOnCelebrate} />);
      }).not.toThrow();
    ...
    it('handles empty metric values', () => {
      ...
      expect(() => {
        render(<MetricCard metric={emptyMetric} index={0} onCelebrate={mockOnCelebrate} />);
      }).not.toThrow();
    ...
    it('handles negative indices gracefully', () => {
      render(<MetricCard metric={mockMetric} index={-1} onCelebrate={mockOnCelebrate} />);
