  ...
  return (
    <div data-testid="accessible-nagarro-page">
      ...
      <main
        id="main-content"
        role="main"
        aria-label="Nagarro Design Leadership Case Study"
      >
          ...
          <nav role="navigation" aria-label="Case study navigation">
            <ul role="list">
              <li role="listitem">
                <a
                  href="#overview"
                  aria-label="Jump to project overview section"
                >
                  Overview
                </a>
              </li>
              <li role="listitem">
                <a href="#metrics" aria-label="Jump to project metrics section">
                  Metrics
                </a>
              </li>
              <li role="listitem">
                <a href="#process" aria-label="Jump to design process section">
                  Process
                </a>
              </li>
            </ul>
          ...
          <dl className="project-metadata" data-testid="project-metadata">
            ...
            <dd>
              <ul role="list" aria-label="Project platforms">
                {mockNagarroAccessibilityData.hero.platforms.map(
                  (platform, index) => (
          ...
          <div className="hero-media" data-testid="hero-media">
            ...
            >
              <p id="video-description">
                Video showcasing the design leadership transformation at
                Nagarro, including team coaching sessions and strategic design
                outcomes.
              </p>
              <p>
...
describe("Nagarro Case Study - WCAG 2.1 AA Accessibility Compliance", () => {
  describe("Automated Accessibility Testing", () => {
    ...
    it.skip("passes accessibility audit for metrics section only", async () => {
      const { container } = render(<MockAccessibleNagarroPage />);
      const metricsSection = container.querySelector(
        '[data-testid="metrics-section"]',
      );
  ...
  describe("Semantic HTML and ARIA Landmarks", () => {
    ...
    it("has proper ARIA landmarks with labels", () => {
      ...
      expect(screen.getByRole("main")).toHaveAttribute(
        "aria-label",
        "Nagarro Design Leadership Case Study",
      );
      expect(
      ...
      expect(
        screen.getByRole("navigation", {
          name: "External case study resources",
        }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("navigation", {
          name: "Case study footer navigation",
        }),
      ).toBeInTheDocument();
    });
    ...
    it("uses proper heading hierarchy", () => {
      ...
      expect(h3Elements.length).toBe(
        mockNagarroAccessibilityData.metrics.length,
      );
    });
  });
  ...
  describe("Form Accessibility", () => {
    it("has properly labeled form controls", () => {
      ...
      expect(ratingSelect).toHaveAttribute(
        "aria-describedby",
        "rating-description",
      );

      const ratingLabel = screen.getByLabelText(
        "Rate this case study (1-5 stars)",
      );
      expect(ratingLabel).toBe(ratingSelect);

      const commentsTextarea = screen.getByTestId("comments-textarea");
      expect(commentsTextarea).toHaveAttribute(
        "aria-describedby",
        "comments-description",
      );

      const commentsLabel = screen.getByLabelText(
        "Additional Comments (optional)",
      );
    ...
    it("has accessible form structure with fieldsets if needed", () => {
      ...
      const formGroups = screen
        .getByTestId("feedback-form")
  ...
  describe("Media Accessibility", () => {
    it("provides accessible video with proper descriptions", () => {
      ...
      expect(heroVideo).toHaveAttribute(
        "aria-label",
        "Nagarro design leadership transformation showcase video",
      );
      expect(heroVideo).toHaveAttribute(
        "aria-describedby",
        "video-description",
      ...
      expect(
        screen.getByText(
          /Video showcasing the design leadership transformation/,
        ),
      ).toBeInTheDocument();
      expect(
    ...
    it("handles video fallbacks for accessibility", () => {
      ...
      expect(videoFallback.closest("a")).toHaveAttribute(
        "href",
        mockNagarroAccessibilityData.hero.heroVideo,
      );
    });
  });
  ...
  describe("Content Structure and Readability", () => {
    ...
    it("provides proper context for metric data", () => {
      ...
      mockNagarroAccessibilityData.metrics.forEach((metric, index) => {
        ...
        expect(metricValue).toHaveAttribute(
          "aria-label",
  ...
  describe("External Link Accessibility", () => {
    it("provides proper context for external links", () => {
      ...
      expect(scribdLink).toHaveAttribute(
        "aria-label",
        "View comprehensive Nagarro design leadership case study on Scribd (opens in new window)",
      );
      ...
      expect(mediumLink).toHaveAttribute(
        "aria-label",
        "Read design leadership insights and lessons learned on Medium (opens in new window)",
      );
    });
