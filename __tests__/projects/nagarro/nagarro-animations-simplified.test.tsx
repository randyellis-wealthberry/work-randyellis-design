...
const SimplifiedNagarroAnimationComponent = ({ enableAnimations = true }: { enableAnimations?: boolean }) => {
  ...
  return (
    <div data-testid="nagarro-animation-component">
      ...
      >
        <h1 data-testid="animated-title">
          {mockNagarroAnimationData.title}
        </h1>
      </header>
      ...
      <section data-testid="metrics-section" className="metrics-container">
        {mockNagarroAnimationData.metrics.map((metric, index) => (
          <article
            key={metric.label}
            className={shouldAnimate ? `animate-slideUp delay-${index * 100}` : ''}
...
describe('Nagarro Case Study - Simplified Animation Tests', () => {
  ...
  describe('Basic Animation Rendering', () => {
    it('renders component with animation elements', () => {
      ...
      expect(screen.getByTestId('nagarro-animation-component')).toBeInTheDocument()
  ...
  describe('Reduced Motion Preference Support', () => {
    ...
    it('maintains functionality when animations are disabled', () => {
      ...
      expect(screen.getByText(mockNagarroAnimationData.title)).toBeInTheDocument()
  ...
  describe('Animation Performance and Accessibility', () => {
    ...
    it('handles component prop changes without errors', () => {
      const { rerender } = render(<SimplifiedNagarroAnimationComponent enableAnimations={true} />)
      
      expect(() => {
        rerender(<SimplifiedNagarroAnimationComponent enableAnimations={false} />)
        rerender(<SimplifiedNagarroAnimationComponent enableAnimations={true} />)
      }).not.toThrow()
    })
  ...
  describe('Error Handling and Fallbacks', () => {
    ...
    it('provides fallbacks when animation props are invalid', () => {
      expect(() => {
        render(<SimplifiedNagarroAnimationComponent enableAnimations={undefined as any} />)
      }).not.toThrow()
    })
    ...
    it('maintains functionality during state changes', () => {
      ...
      expect(() => {
        for (let i = 0; i < 10; i++) {
          rerender(<SimplifiedNagarroAnimationComponent enableAnimations={i % 2 === 0} />)
        }
      }).not.toThrow()
    })
  })
  ...
  describe('Content Validation', () => {
    it('displays all required Nagarro content', () => {
      ...
      expect(screen.getByText('Designer Skills Enhancement')).toBeInTheDocument()
      expect(screen.getByText('50%')).toBeInTheDocument()
      expect(screen.getByText('100+')).toBeInTheDocument()
    ...
    it('maintains proper heading structure', () => {
      ...
      expect(metricHeadings).toHaveLength(mockNagarroAnimationData.metrics.length + 1) // metrics + scroll content
    })
  })
})