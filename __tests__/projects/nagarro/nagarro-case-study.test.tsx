  ...
  sections: [
    {
      id: 'challenge',
      title: 'The Challenge',
      content: 'Transforming a traditional IT consulting firm to embrace strategic design thinking while maintaining operational excellence and client satisfaction.'
    },
    {
      id: 'approach',
      title: 'Strategic Approach', 
      content: 'Implemented a three-pillar strategy: design framework development, team capability enhancement, and thought leadership establishment.'
    },
    {
      id: 'process',
      title: 'Design Leadership Process',
      content: 'Coached 15+ designers through strategic thinking development while executing measurable brand recognition and lead generation initiatives.'
    },
    {
      id: 'impact',
      title: 'Business Impact',
      content: 'Achieved 50% brand recognition increase, 100+ qualified leads, and 40% improvement in team retention through strategic design leadership.'
    }
  ]
}

// Mock CaseStudyLayout component following Echo pattern
const MockNagarroCaseStudyLayout = ({ data }: { data: typeof mockNagarroDataForCaseStudy }) => {
  return (
    <main role="main" className="w-full case-study-layout">
      <div data-testid="case-study-container">
          ...
          <div className="hero-content">
            ...
            <div className="hero-metadata" data-testid="hero-metadata">
              <div className="metadata-item">
                ...
                <span className="value" data-testid="client-value">{data.hero.client}</span>
              </div>
              <div className="metadata-item">
                <span className="label">Timeline:</span>
                <span className="value" data-testid="timeline-value">{data.hero.timeline}</span>
              </div>
              <div className="metadata-item platforms" data-testid="platforms-container">
                <span className="label">Platforms:</span>
                <div className="platform-badges" role="list" aria-label="Project platforms">
                  {data.hero.platforms.map((platform, index) => (
                    <span key={index} className="badge platform-badge" role="listitem">
                      {platform}
                    </span>
                  ))}
                </div>
        ...
        {/* Metrics Section */}
        <section data-testid="metrics-section" className="metrics-section" aria-label="Project metrics">
          ...
          <div className="metrics-grid" data-testid="metrics-grid">
            {data.metrics.map((metric, index) => (
              ...
              >
                <h3 id={`metric-label-${index}`} className="metric-label">{metric.label}</h3>
                <div className="metric-value" data-testid={`metric-value-${index}`}>{metric.value}</div>
        ...
        <section data-testid="external-links-section" className="external-links-section">
          <h2 className="section-title">Additional Resources</h2>
          <div className="external-links" role="list" aria-label="External resources">
            <a 
            ...
            >
              ...
              <span className="link-description">Detailed documentation with process insights</span>
            </a>
            ...
            >
              ...
              <span className="link-text">Design Leadership Article (Medium)</span>
              <span className="link-description">Strategic insights and lessons learned</span>
            </a>
          </div>
        </section>

        {/* Navigation/Scroll to Top */}
        <div className="case-study-navigation" data-testid="case-study-navigation">
          <button 
...
describe('Nagarro Case Study Layout - TDD Component Tests', () => {
  ...
  describe('Hero Section Functionality', () => {
    it('displays hero content with correct metadata', () => {
      ...
      expect(screen.getByText(mockNagarroDataForCaseStudy.hero.title)).toBeInTheDocument()
      expect(screen.getByText(mockNagarroDataForCaseStudy.hero.subtitle)).toBeInTheDocument()
    })

    it('renders client and timeline information correctly', () => {
      render(<MockNagarroCaseStudyLayout data={mockNagarroDataForCaseStudy} />)
      
      expect(screen.getByTestId('client-value')).toHaveTextContent('Nagarro Software Engineering GmbH')
      expect(screen.getByTestId('timeline-value')).toHaveTextContent('Mar 2022 - Oct 2022 (8 months)')
    })
    ...
    it('displays platform badges with proper accessibility', () => {
      ...
      expect(platformBadges.length).toBeGreaterThanOrEqual(mockNagarroDataForCaseStudy.hero.platforms.length)
    ...
    it('renders hero video with proper accessibility attributes', () => {
      ...
      expect(heroVideo).toHaveAttribute('src', mockNagarroDataForCaseStudy.hero.heroVideo)
      expect(heroVideo).toHaveAttribute('poster', mockNagarroDataForCaseStudy.hero.heroImage)
      expect(heroVideo).toHaveAttribute('controls')
      expect(heroVideo).toHaveAttribute('aria-label', 'Nagarro design leadership showcase video')
    })
  })
  ...
  describe('Metrics Section Functionality', () => {
    ...
    it('displays metric values and descriptions correctly', () => {
      ...
      mockNagarroDataForCaseStudy.metrics.forEach((metric, index) => {
        ...
        expect(screen.getByTestId(`metric-value-${index}`)).toHaveTextContent(metric.value)
  ...
  describe('Content Sections Functionality', () => {
    it('renders all case study sections with proper IDs', () => {
      ...
      mockNagarroDataForCaseStudy.sections.forEach(section => {
        ...
        expect(sectionElement).toHaveAttribute('aria-labelledby', `section-title-${section.id}`)
      })
    })
  ...
  describe('External Links Integration', () => {
    it('renders external links section with proper structure', () => {
      ...
      const externalLinksList = screen.getByRole('list', { name: 'External resources' })
    ...
    it('displays Scribd link with correct attributes and content', () => {
      ...
      expect(scribdLink).toHaveAttribute('href', 'https://www.scribd.com/document/nagarro-design-leadership-case-study')
      expect(scribdLink).toHaveAttribute('target', '_blank')
      expect(scribdLink).toHaveAttribute('rel', 'noopener noreferrer')
      expect(scribdLink).toHaveAttribute('aria-label', 'View comprehensive case study documentation on Scribd')
      
      expect(screen.getByText('Complete Case Study (Scribd)')).toBeInTheDocument()
      expect(screen.getByText('Detailed documentation with process insights')).toBeInTheDocument()
    })
    ...
    it('displays Medium link with correct attributes and content', () => {
      ...
      expect(mediumLink).toHaveAttribute('href', 'https://medium.com/@randyellis/nagarro-design-leadership-transformation')
      expect(mediumLink).toHaveAttribute('target', '_blank')
      expect(mediumLink).toHaveAttribute('rel', 'noopener noreferrer')
      expect(mediumLink).toHaveAttribute('aria-label', 'Read design leadership insights and lessons learned on Medium')
      
      expect(screen.getByText('Design Leadership Article (Medium)')).toBeInTheDocument()
      expect(screen.getByText('Strategic insights and lessons learned')).toBeInTheDocument()
    })
  ...
  describe('Navigation and Interaction', () => {
    it('renders scroll to top button with proper functionality', async () => {
      ...
      expect(scrollToTopBtn).toHaveAttribute('aria-label', 'Scroll to top of page')
