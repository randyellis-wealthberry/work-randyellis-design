...
const mockNagarroProject: Project = {
  ...
  description:
    "Led design strategy transformation at global IT consulting firm, boosting brand recognition by 50% and generating 100+ qualified leads through strategic design thinking, team development, and thought leadership initiatives.",
  longDescription:
  ...
  processStory: {
    ...
    stakeholderQuotes: [
      {
        quote:
          "Randy transformed our design team from execution-focused to strategy-driven, creating measurable business impact that elevated our competitive position in the IT consulting market.",
        author: "Dr. Sarah Mueller",
        role: "VP of Digital Services, Nagarro",
      },
      {
        quote:
          "The coaching program didn't just improve my design skills - it taught me how to think strategically about business problems and communicate design value to executives.",
        author: "Marcus Chen",
        role: "Senior UX Designer, Nagarro",
      },
      {
        quote:
          "The thought leadership content strategy generated more qualified leads than any previous marketing initiative, establishing our design expertise as a key differentiator.",
...
describe("TDD: Nagarro Design Leadership Data Validation", () => {
  describe("Project Data Structure Integrity", () => {
    ...
    test("should have valid media references", () => {
      ...
      expect(
        mockNagarroProject.thumbnail!.startsWith("/projects/nagarro/"),
      ).toBe(true);
    ...
    test("should have valid timeline and project metadata", () => {
      ...
      expect(["completed", "in-progress", "planned"]).toContain(
        mockNagarroProject.status,
      );
  ...
  describe("Business Impact Validation", () => {
    test("should have measurable business impact metrics", () => {
      const impactMetrics = mockNagarroProject.metrics!.filter(
        (metric) => metric.performanceLevel === "excellent",
      );
      expect(impactMetrics.length).toBeGreaterThan(0);

      // Verify key business metrics exist
      const brandRecognitionMetric = mockNagarroProject.metrics!.find(
        (m) => m.label === "Brand Recognition Increase",
      );
      expect(brandRecognitionMetric).toBeDefined();
      expect(brandRecognitionMetric?.value).toBe("50%");

      const leadsMetric = mockNagarroProject.metrics!.find(
        (m) => m.label === "Qualified Leads Generated",
      );
  ...
  describe("Process Story Validation", () => {
    test("should have comprehensive process story structure", () => {
      ...
      expect(mockNagarroProject.processStory!.outcome!.length).toBeGreaterThan(
        100,
      );
    ...
    test("should have key insights array with meaningful content", () => {
      ...
      expect(Array.isArray(mockNagarroProject.processStory!.keyInsights!)).toBe(
        true,
      );
      expect(
        mockNagarroProject.processStory!.keyInsights!.length,
      ).toBeGreaterThan(0);
    ...
    test("should have stakeholder quotes with proper attribution", () => {
      ...
      expect(
        Array.isArray(mockNagarroProject.processStory!.stakeholderQuotes!),
      ).toBe(true);
      expect(
        mockNagarroProject.processStory!.stakeholderQuotes!.length,
      ).toBeGreaterThan(0);
  ...
  describe("Overview and Constraints Validation", () => {
    test("should have detailed overview structure", () => {
      ...
      expect(Array.isArray(mockNagarroProject.overview!.deliverables)).toBe(
        true,
      );
      expect(mockNagarroProject.overview!.deliverables.length).toBeGreaterThan(
        0,
      );
    ...
    test("should have comprehensive constraints documentation", () => {
      ...
      expect(Array.isArray(mockNagarroProject.constraints!.technical)).toBe(
        true,
      );

      expect(mockNagarroProject.constraints!.environmental).toBeDefined();
      expect(Array.isArray(mockNagarroProject.constraints!.environmental)).toBe(
        true,
      );
  ...
  describe("Integration with Existing Work Experience", () => {
    test("should align with Nagarro work experience data", () => {
      const nagarroWorkExperience = WORK_EXPERIENCE.find(
        (work) => work.company === "Nagarro",
      );
    ...
    test("should validate timeline consistency", () => {
      ...
      expect(mockNagarroProject.overview!.timelineDuration).toContain(
        "8 months",
      );
    });
  });
  ...
  describe("Design Leadership Focus Validation", () => {
    ...
    test("should demonstrate measurable business outcomes", () => {
      const businessImpactMetrics = mockNagarroProject.metrics!?.filter(
        (metric) =>
