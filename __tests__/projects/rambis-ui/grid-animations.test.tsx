...
jest.mock("motion/react", () => {
  ...
  const mockUseTransform = jest.fn((value, transformer) => {
    ...
    if (transformer && typeof transformer === 'function') {
      return transformer(typeof value === 'object' && value.get ? value.get() : value || 0);
    }
    return typeof value === 'object' && value.get ? value.get() : value || 0;
  });

  const createMotionComponent = (element: string) => {
    return ({ children, className, style, onMouseMove, onMouseEnter, onMouseLeave, initial, animate, whileInView, variants, viewport, transition, onAnimationComplete, ...props }: any) => {
...
jest.mock("@/components/motion-primitives/glare-hover", () => ({
  GlareHover: ({ children, className, disabled, intensity, onMouseMove, onMouseEnter, onMouseLeave, glareColor, glareOpacity, glareSize, borderRadius, ...props }: any) => {
...
jest.mock("@/components/motion-primitives/animated-content", () => ({
  AnimatedContent: ({ children, className, staggerDelay, staggerDirection, duration, once, threshold, as = "div", ...props }: any) => {
    const Component = as;
    return (
      <Component 
        className={className}
...
describe("Rambis UI Grid Animations", () => {
  ...
  describe("Motion Components Integration", () => {
    it("renders motion.div components with proper attributes", () => {
      ...
      const motionDivs = container.querySelectorAll("[data-motion-component='true']");
    ...
    it("handles animation variants properly", () => {
      ...
      const elementsWithVariants = container.querySelectorAll("[data-variants='true']");
  ...
  describe("GlareHover Effects", () => {
    it("applies glare hover effects to interactive cards", () => {
      ...
      const glareElements = container.querySelectorAll("[data-glare-hover='true']");
  ...
  describe("AnimatedContent Integration", () => {
    it("applies animated content wrapper", () => {
      ...
      const animatedContent = container.querySelectorAll("[data-animated-content='true']");
    ...
    it("configures stagger animations properly", () => {
      ...
      const animatedElements = container.querySelectorAll("[data-animated-content='true']");
      ...
      animatedElements.forEach(element => {
        ...
        if (staggerDirection) {
          expect(["top", "bottom", "left", "right"]).toContain(staggerDirection);
        }
      });
    });
    ...
    it("supports different semantic elements", () => {
      ...
      const tagNames = Array.from(animatedElements).map(el => el.tagName.toLowerCase());
  ...
  describe("Animation Performance", () => {
    ...
    it("applies will-change property for optimized animations", () => {
      ...
      const elementsWithWillChange = Array.from(container.querySelectorAll("*")).filter(el => {
