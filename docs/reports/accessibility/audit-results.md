...
export default function RootLayout({ children }) {
  return (
    <MotionConfig reducedMotion="user">{/* existing layout */}</MotionConfig>
  );
}
```
...
>
  ...
  <div role="alert" aria-live="polite" aria-atomic="true">
    {submitStatus === "error" && <p className="text-red-600">{errorMessage}</p>}
  </div>
...
_This audit was conducted according to WCAG 2.1 AA guidelines and includes considerations for upcoming EAA 2025 compliance requirements. For questions or clarifications, please contact the UX Research team._
